...
class Updater extends Base
{
	...
	protected function getDownloadUrl()
	{
		...
		if (
			!is_array($this->githubApiResult->assets) ||
			!count($this->githubApiResult->assets)
		) {
	...
	protected function getRepoReleaseInfo()
	{
		...
		if (
			!is_array($this->githubApiResult) ||
			!count($this->githubApiResult)
		) {
			return;
		}
	...
	public function setTransient($transient)
	{
		...
		$doUpdate = version_compare(
			...
			$transient->checked[$this->context->basename()]
		);
